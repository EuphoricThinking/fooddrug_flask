...
def create_app(test_config=None):
	...
	@app.route("/listDrugs", methods=["POST", "GET"])
	def listDrugs():
		data = db.get_db()
		data.row_factory = sqlite3.Row
		cur = data.cursor()
		cur.execute("select Nazwa_handlowa from lek;")
		rows = cur.fetchall()

		return render_template("listDataLinks.html", rows = rows)

	@app.route("/dataDrugList", methods=["POST", "GET"])
	def dataDrugList():
		data = db.get_db()
		print("robie cos", flush=True)
		if request.method == "POST":
			try:
				...
				if len(rows) == 0:
					...
					return render_template("wypiszDaneZListy.html", rows=rows, name=name,
